 ...
 *
 */
public class WcsOrderStsUpdateMessageListener implements MessageListener {
	
	private static final Logger logger = LoggerFactory.getLogger(WcsOrderStsUpdateMessageListener.class);
	...
	public void onMessage(Message message, byte[] chaanel) {
		...
		logger.debug("WCS OrderUpdate Listener: " + message.toString() + " from Channel [" + new String(chaanel) +"]");
		...
		try {
			...
			// Set Database Index
//			wcsStringRedisTemplate.getConnectionFactory().getConnection().select(10);
//			ListOperations<String, String> listOps = wcsStringRedisTemplate.opsForList();
			
			// 2. Get Data Count by Key
//			List<String> list = listOps.range(key, 0, -1);
//			logger.debug("[list.size-read]"+list.size());
			...
			for(int i=0; i<listOps.size(key); i++){
				...
				if("3201".equals(status)){
					...
					for(int j=0; j<releaseKeys.size(); j++){
						...
						// Sterling API Call
						HashMap<String, String> resultMap = sterlingApiDelegate.createShipment("", releaseKeys.get(i));
